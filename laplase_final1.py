...
file=open("training_sentences.txt","r")
print("\n\n#################### TRAINING ######################")
print("\nTokenizing ...")
...
for w1,w2 in bigram_list:
	if(model1[w1][w2]!=0):
		tokens_bigram=tokens_bigram+1


for w1 in model1:
	total_count = float(sum(model1[w1].values()))
	for w2 in model1[w1]:
		model1[w1][w2] = (model1[w1][w2]+1)/(total_count+tokens_bigram)
...
for w1 in model2:
	model2[w1]=(model2[w1]+1)/(unicount+tokens)
	#print(model2[w1])
...
tokens_trigram=0
for w1,w2,w3 in trigram_list:
	if(model1[(w1,w2)][w3]!=0):
		tokens_trigram=tokens_trigram+1


for w1_w2 in model:    
	total_count = float(sum(model[w1_w2].values()))
	for w3 in model[w1_w2]:
		model[w1_w2][w3] = (model[w1_w2][w3]+1)/(total_count+tokens_trigram)
...
file1=open("testing_sentences.txt","r")
print("\nTokenizing ...")
...
for w1,w2,w3 in trigram_list:
	...
	if(model[(w1,w2)][w3]!=0):
		...
		if(model1[w2][w3]!=0):
			...
			dict1[(w1,w2,w3)]=model1[w2][w3]
		elif(model2[temp]!=0):
			count_unigram=count_unigram+1
			dict1[(w1,w2,w3)]=model2[temp]
			#print(model2[temp])
		else:
			count_UNK=count_UNK+1
			dict1[(w1,w2,w3)]=1/(unicount+tokens)
...
for i in dict1.values():
	prob_final=prob_final + math.log(i,2)
	likelihood += math.log(i,2)

f1 = open("laplace_full_main.txt","w")
...
f1.write("Trigram Count:")
f1.write(str(count_trigram))
f1.write("\n")
f1.write("Bigram Count:")
f1.write(str(count_bigram))
f1.write("\n")
f1.write("Unigram Count:")
f1.write(str(count_unigram))
f1.write("\n")
f1.write("UNK Count:")
f1.write(str(count_UNK))
f1.write("\n")
f1.write("Perplexity:")
f1.write(str(PP))
f1.write("\n")
